...
async def _score_one_dim(
    ...
    """
    try:
        # 8192 for reasoning models (Minimax M3, DeepSeek R1, OpenAI o3,
        # Anthropic extended-thinking, etc.) that spend most of their output
        # budget on internal <thinking> tokens before emitting the visible JSON.
        # Observed empirically: Minimax M3 burns 2000+ tokens on reasoning,
        # leaving insufficient room for structured output at lower limits.
        # Non-reasoning models are unaffected because billing only counts
