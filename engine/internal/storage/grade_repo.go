...
func (s *Store) SaveGrade(ctx context.Context, grade models.Grade) error {
	...
	if grade.BaselineID != "" {
		if _, err := s.DB.ExecContext(ctx, `DELETE FROM grades WHERE baseline_id = ?`, grade.BaselineID); err != nil {
			return fmt.Errorf("delete existing baseline grade: %w", err)
		}
	}
	_, err := s.DB.ExecContext(ctx, `
		INSERT INTO grades (
			id, run_id, baseline_id, test_pass_rate, test_pass_count, test_fail_count, lint_score, type_check_pass,
	...
	`, grade.ID, nullableString(grade.RunID), nullableString(grade.BaselineID), grade.TestPassRate, grade.TestPassCount, grade.TestFailCount, grade.LintScore,
...
func (s *Store) GetGradeByRun(ctx context.Context, runID string) (*models.Grade, error) {
	row := s.DB.QueryRowContext(ctx, `
		SELECT id, run_id, baseline_id, test_pass_rate, test_pass_count, test_fail_count, lint_score, type_check_pass,
		       file_state_valid, turn_count, total_tokens, cost_usd, token_efficiency, backtrack_count,
		       self_validation_rate, premature_completion, idle_turns, error_recovery_count, tool_call_accuracy,
		       context_utilization, judge_correctness, judge_maintainability, judge_completeness, judge_best_practices,
		       judge_error_handling, judge_irr_alpha, raw_judge_responses_json, spec_instruction_compliance,
	...
	if err := row.Scan(&grade.ID, &grade.RunID, &baselineID, &grade.TestPassRate, &grade.TestPassCount, &grade.TestFailCount, &grade.LintScore,
	...
	grade.BaselineID = baselineID.String
