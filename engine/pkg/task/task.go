...
// Task is the public spec for an evaluation task.
//
// A task directory on disk maps to this struct via Loader (TODO: post-MVP).
// In MVP, tasks are loaded from tasks/<id>/task.yaml plus a sibling tests/
// directory and setup.sh/eval.sh scripts.
type Task struct {
...
// TestCase is a single deterministic test inside a task's test suite.
//
// The Command is executed inside the sandbox by eval.sh after the agent
// completes; exit code 0 means the test passed.
