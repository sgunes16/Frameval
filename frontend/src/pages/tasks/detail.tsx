  ...
  return (
    <div className="space-y-4">
      <Card>
        ...
        <p className="mt-4 whitespace-pre-wrap text-sm text-fg-muted">{task.description}</p>
      ...
      <Card>
        ...
        <pre className="whitespace-pre-wrap rounded-lg bg-bg-elev-2 p-4 text-sm leading-6 text-fg">{task.task_prompt}</pre>
      </Card>
      <Card>
        ...
        <div className="space-y-2">
          {(task.test_cases ?? []).map((test) => (
            <div key={test.name} className="rounded-lg border border-border p-3 text-sm">
              <div className="flex items-center gap-2">
                <div className="font-medium text-fg">{test.name}</div>
                {test.visibility === 'hidden' && <Badge tone="warning">hidden</Badge>}
              </div>
              {test.test_command ? (
                <code className="mt-1 block rounded bg-bg-elev-2 px-2 py-1 text-xs text-fg">{test.test_command}</code>
              ) : (
                <div className="mt-1 rounded bg-bg-elev-2 px-2 py-1 text-xs text-fg-muted">Hidden verification command is not exposed in the UI.</div>
              )}
            </div>
          ))}
          {(!task.test_cases || task.test_cases.length === 0) && (
            <div className="rounded-lg border border-dashed border-border p-4 text-xs text-fg-muted">
