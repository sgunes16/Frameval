...
def test_grade_uses_rubrics_from_proto():
    ...
    with patch("grader.llm_judge.grader.build_client_with_cleanup", return_value=(stub, _noop_close)):
...
def test_grade_client_init_failure_returns_all_failed():
    ...
    with patch("grader.llm_judge.grader.build_client_with_cleanup", side_effect=RuntimeError("no key")):
        out = grade({}, {}, task={}, output_files=[], transcript_json=b"", config_override=cfg)
...
def test_grade_config_override_beats_env(monkeypatch):
    ...
    def fake_build(cfg, *, async_client):
        ...
        return stub, _noop_close

    with patch("grader.llm_judge.grader.build_client_with_cleanup", side_effect=fake_build):
