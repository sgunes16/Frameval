...
func TestMain(m *testing.M) {
	goleak.VerifyTestMain(m,
		// gRPC's internal "grpc/grpclog" goroutines and the underlying go-grpc
		// connectivity poller are package-level singletons that legitimately
		// outlive a single test. Ignoring them here keeps the leak budget tight
		// while not flagging library-owned background work.
...
func TestGraderClient_ClassifyFailure_RoundTripsThroughFakeGrader(t *testing.T) {
	...
	if verdict.Classification.Confidence != 0.88 {
		t.Errorf("Confidence: want 0.88, got %v", verdict.Classification.Confidence)
	}
}
